...
import { FriendType } from '../types/model/friend.model.types';
...
export class Friend {
  constructor(
    private friend_initiator_id?: number,
    private friend_one_id?: number,
    private friend_two_id?: number
  ) {}
  ...
  async create() {
    ...
    try {
      const friendOne = await new User(
        undefined,
        undefined,
        undefined,
        undefined,
      ...
      const friendTwo = await new User(
        undefined,
        undefined,
        undefined,
        undefined,
      ...
      if (existingFriend?.rows[0]) {
        if (
          this.friend_one_id === existingFriend?.rows[0].friend_initiator_id
        ) {
  ...
  serializeFriend(friend: QueryResult) {
    return {
      id: friend.rows[0].id,
